import { CambridgeScraper } from '../../../src';
import { DictionaryEntry } from '../../../src/types/word';
import CambridgeReader from '../../fake/cambridge-reader';
import TimeoutReader from '../../fake/timeout-reader';

describe('CambridgeScraper::scrape()', () => {
  ...
  test('scrape should return expected data for single region', async () => {
    const ret = await scraper.scrape(`${VALID_SINGLE_PURPOSE_WORD}-us`);
  ...
  test('search should return expected data for valid multi category word', async () => {
    const ret = await scraper.scrape(VALID_MULTI_CATEGORY_WORD);
  ...
  test('scrape should return undefined for nonsensical word', async () => {
    const ret = await scraper.scrape(NONSENSICAL_WORD);
  ...
  test('timeout should throws exception', async () => {
    ...
    await expect(
      scraper.scrape(VALID_SINGLE_PURPOSE_WORD),
  ...
  test.each([
    ...
    [{ key: 'value' }],
    [() => {}],
  ])('given %s, scrape should throw error', async (invalidWord) => {
